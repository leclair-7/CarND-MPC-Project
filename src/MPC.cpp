 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      ...
      fg[0] +=  velo_weight * CppAD::pow(vars[v_start + t] - ref_v, 2);
    }
    ...
    for (int t = 0; t < N - 2; t++) {
      fg[0] +=  steer_succesive * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);
    ...
    for (int t = 1; t < N; t++) {
      ...
      // The idea here is to constrain this values to 0.
      //
      // NOTE: The use of `AD<double>` and use of `CppAD`!
      // CppAD can compute derivatives and pass
      // the values to the solver.
      ...
      //This has to be psi0 - vo/Lf because simulator steer coordinate directions
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // Set all non-actuators upper/lower limits to [verysmall..verybig]
  ...
  // degrees (bounds are in radians).  
  for (int i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436332;
    vars_upperbound[i] = 0.436332;
  }

  // Acceleration/decceleration upper and lower limits. Put 0 as lower bound because vehicle braking looks weird
  ...
  // Return actuator values. The variables can be accessed with
  // `solution.x[i]`.
  //
